...
namespace BuleyeanMath.Arithmetic
...
-- Distribution
theorem mul_add (a b c : Nat) : a * (b + c) = a * b + a * c := Nat.mul_add a b c
theorem add_mul (a b c : Nat) : (a + b) * c = a * c + b * c := Nat.add_mul a b c
...
theorem mul_sub (a b c : Nat) : a * (b - c) = a * b - a * c := Nat.mul_sub_left_distrib a b c

-- BuleReal specific identities (Scale: 10^9)
def one : BuleReal := BuleReal.scale
def zero : BuleReal := 0

theorem scale_pos : 0 < BuleReal.scale := by native_decide

theorem ofNat_monotone {a b : Nat} (h : a <= b) :
    BuleReal.ofNat a <= BuleReal.ofNat b := by
  unfold BuleReal.ofNat
  exact Nat.mul_le_mul_right BuleReal.scale h

end BuleyeanMath.Arithmetic
