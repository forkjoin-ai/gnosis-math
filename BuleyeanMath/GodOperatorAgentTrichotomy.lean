...
namespace GodOperatorAgentTrichotomy
...
theorem god_characterized_not_computed :
    godPosition.characterization ≠ "" := by decide

/-! ## The type-level separation

Lean's type theory keeps these separate by construction. An `Agent`
is not a `GodsPosition` and cannot be coerced to one. An `Agent` is
...
- A `GodsPosition` (characterized, not computed).
...
`Nat → Nat` function is expected; a `GodsPosition` cannot be passed
...
The God position IS unique: one `GodsPosition` value is constructed,
...
by a theorem. `Agent`, `Nat → Nat` (the Operator), and `GodsPosition`
...
end GodOperatorAgentTrichotomy
end BuleyeanMath
