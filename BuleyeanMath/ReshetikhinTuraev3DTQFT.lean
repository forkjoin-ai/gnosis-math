...
-- ══════════════════════════════════════════════════════════
-- GENUINE  (ST)³ = S²  AT  q  A  PRIMITIVE  24th  ROOT (additive)
-- ══════════════════════════════════════════════════════════
-- The original `T1_sq_diag` and `S1_sq_is_2I` work over ℤ, treating
-- the modular T-matrix as ±1 — a coarse parity shadow that hides
-- the genuine 24th-root-of-unity entries.  Below we promote the
-- modular relation `(ST)³ = S²` for SU(2)_1 to an honest matrix
-- identity in `Cyc 24` by absorbing the `1/√2` normalisation into
-- the squared form  `(S'T)^6 = 8 · I`,  where  S' = √2 · S  is
-- the integer matrix [[1,1],[1,-1]] and T = diag(ζ_24^a, ζ_24^b)
-- carries the genuine root-of-unity entries.
--
-- Identity used:
--   * S = S'/√2,  so  S² = (S')²/2 = I  (since (S')² = 2I).
--   * Hence  (ST)³ = S² = I  and squaring gives  (ST)^6 = I.
--   * In integer-clearing form: (√2)^6 (ST)^6 = 8 (ST)^6 = (S'T)^6,
--     and we want this to equal 8 · I.
--
-- Choice of phases (SU(2)_1 with c = 1, h₀ = 0, h₁ = 1/4):
--   θ_0 = e^{-2πi · 1/24}      = ζ_24^{-1} = ζ_24^{23}
--   θ_1 = e^{2πi(1/4 - 1/24)}  = ζ_24^{5}.

open ForkRaceFoldMath

/-- 2 × 2 matrix over `Cyc 24`. -/
abbrev MatC24 := Fin 2 → Fin 2 → Cyc 24

def matC24 (a b c d : Cyc 24) : MatC24 :=
  fun i j =>
    match i, j with
    | ⟨0, _⟩, ⟨0, _⟩ => a
    | ⟨0, _⟩, ⟨1, _⟩ => b
...
def matC24Pow (M : MatC24) : Nat → MatC24
  | 0     => matC24 (Cyc.one 24) (Cyc.zero 24) (Cyc.zero 24) (Cyc.one 24)
  | k + 1 => matC24Mul M (matC24Pow M k)

/-- ζ_24 ∈ Cyc 24. -/
def zeta24 : Cyc 24 := Cyc.zeta 24

/-- S' = √2 · S  =  [[1,1],[1,-1]]  (integer matrix, lifted to Cyc 24). -/
def S_SU2_1 : MatC24 :=
  matC24 (Cyc.one 24) (Cyc.one 24) (Cyc.one 24) (Cyc.neg (Cyc.one 24))

/-- T-matrix diagonal entry  θ_0 = e^{-2πi · c/24} = ζ_24^{-1} = ζ_24^{23}.
    With central charge c = 1 for SU(2)_1. -/
def T_SU2_1_00 : Cyc 24 := zeta24.pow 23

/-- T-matrix diagonal entry  θ_1 = e^{2πi(h_1 - c/24)} where h_1 = 1/4,
    c = 1, giving exponent 1/4 - 1/24 = 5/24.  So θ_1 = ζ_24^5. -/
def T_SU2_1_11 : Cyc 24 := zeta24.pow 5

/-- T = diag(θ_0, θ_1) for SU(2)_1. -/
def T_SU2_1 : MatC24 :=
  matC24 T_SU2_1_00 (Cyc.zero 24) (Cyc.zero 24) T_SU2_1_11

/-- Scalar `c` lifted to a diagonal matrix `c · I`. -/
def scalarMat (c : Cyc 24) : MatC24 :=
  matC24 c (Cyc.zero 24) (Cyc.zero 24) c

/-- Integer scalar `n` lifted via Cyc 24 representation.  Convenience helper. -/
def intScalarMat (n : Int) : MatC24 :=
  scalarMat ⟨[n, 0, 0, 0, 0, 0, 0, 0]⟩

/-- Identity matrix in Cyc 24. -/
def matI24 : MatC24 := intScalarMat 1

/-- 2 · I, 4 · I, 8 · I, 4096 · I as convenience constants. -/
def twoI    : MatC24 := intScalarMat 2
def fourI   : MatC24 := intScalarMat 4
def eightI  : MatC24 := intScalarMat 8
def m4096I  : MatC24 := intScalarMat 4096

/-- (S')² = 2I.  Sanity check on the unnormalised S-matrix. -/
theorem S_prime_sq_eq_2I :
    matC24Eq (matC24Pow S_SU2_1 2) twoI = true := by native_decide

/-- (S')⁴ = 4I.  Hence the genuine S² = I as expected. -/
theorem S_prime_pow4_eq_4I :
    matC24Eq (matC24Pow S_SU2_1 4) fourI = true := by native_decide

/-- T_SU2_1 has order 24 (with the central-charge phase included).  This
    is a strict refinement of the integer ±1 parity shadow `T1_sq_diag`. -/
theorem T_SU2_1_pow24_eq_I :
    matC24Eq (matC24Pow T_SU2_1 24) matI24 = true := by native_decide

/-- T_SU2_1 raised to a smaller power is *not* the identity (sample). -/
theorem T_SU2_1_pow12_ne_I :
    matC24Eq (matC24Pow T_SU2_1 12) matI24 = false := by native_decide

/-- **(ST)³ = ζ_8 · S²  in Cyc 24** — the genuine modular relation
    for an MTC with central charge c = 1.  In the cleared squared form
    this becomes
        (S'T)^6   =   8 · ζ_24^6 · I
    where ζ_24^6 = e^{2πi · 6/24} = i = ζ_4 — the squared phase
    `(e^{2πi c/8})^2 = e^{iπ c/2} = i` for c = 1.

    Equivalently, (S'T)^24 = 4096 · I (the 24th-power form has no phase),
    proved as `ST_to_24_eq_4096I` below. -/
theorem ST_cubed_eq_S_squared :
    matC24Eq (matC24Pow (matC24Mul S_SU2_1 T_SU2_1) 6)
             (scalarMat ⟨[0, 0, 0, 0, 0, 0, 8, 0]⟩) = true := by
  native_decide

/-- The phase-free form: (S'T)^24 = 4096 · I.  Derivation:
       (ST)^3  = ζ_8 S²
       (ST)^24 = ζ_8^8 · S^16 = 1 · I = I
       (S'T)^24 = (√2)^24 · I = 2^12 · I = 4096 · I. -/
theorem ST_to_24_eq_4096I :
    matC24Eq (matC24Pow (matC24Mul S_SU2_1 T_SU2_1) 24) m4096I = true := by
  native_decide

-- A finer check: the unsquared relation (S'T)^3 = (some scalar) · (S')²
-- requires √2 inside Cyc 24, which forces rational coefficients.  We
-- stick with the squared-and-cleared form above and verify the
-- complementary unitarity statement below.

/-- Conjugate-transpose for entries in Cyc 24: complex conjugation
    sends ζ_24 ↦ ζ_24^{-1} = ζ_24^{23}.  On a Cyc 24 element
    `Σ aᵢ ζ_24^i`, conjugation gives `Σ aᵢ ζ_24^{-i}`.  Equivalently,
    we represent it by replacing `[a₀, a₁, ..., a₇]` with the
    coefficient list of `Σ aᵢ ζ_24^{-i}` reduced modulo Φ_24. -/
def conjC24 (c : Cyc 24) : Cyc 24 :=
  -- Σ aᵢ · (ζ_24^{-1})^i  =  Σ aᵢ · ζ_24^{24-i}.  We compute by
  -- summing  aᵢ · zeta24.pow (24 - i)  for i = 0..7 (the relevant range).
  let rec go (i : Nat) (xs : List Int) (acc : Cyc 24) : Cyc 24 :=
    match xs with
    | []      => acc
    | a :: t =>
      let term : Cyc 24 :=
        let zPow := if 24 ≥ i then zeta24.pow (24 - i) else Cyc.one 24
        ⟨zPow.coeffs.map (a * ·)⟩
      go (i + 1) t (Cyc.add acc term)
  go 0 c.coeffs (Cyc.zero 24)

/-- Conjugate transpose of a 2 × 2 Cyc 24 matrix. -/
def matC24Adj (M : MatC24) : MatC24 :=
  matC24 (conjC24 (M ⟨0, by decide⟩ ⟨0, by decide⟩))
         (conjC24 (M ⟨1, by decide⟩ ⟨0, by decide⟩))
         (conjC24 (M ⟨0, by decide⟩ ⟨1, by decide⟩))
         (conjC24 (M ⟨1, by decide⟩ ⟨1, by decide⟩))

/-- **S genuinely unitary** in Cyc 24:  S · S† = I  in the cleared form
    (S')(S')† = 2 · I  (the √2 normalisation factors give 2 on the RHS,
    not 1).  This is the cleared statement of S unitarity. -/
theorem S_SU2_1_unitary_genuine :
    matC24Eq (matC24Mul S_SU2_1 (matC24Adj S_SU2_1)) twoI = true := by
  native_decide

/-- Combined genuine modular shadow: (S'T)^24 = 4096 · I, S unitary
    (S')(S')† = 2I, and T order 24 — upgrading both `S1_sq_is_2I` and
    `T1_sq_diag` from integer parity to honest 24th-root-of-unity arithmetic. -/
theorem RT_modular_genuine :
      matC24Eq (matC24Pow (matC24Mul S_SU2_1 T_SU2_1) 24) m4096I = true
    ∧ matC24Eq (matC24Mul S_SU2_1 (matC24Adj S_SU2_1)) twoI = true
    ∧ matC24Eq (matC24Pow T_SU2_1 24) matI24 = true := by
