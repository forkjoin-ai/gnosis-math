...
namespace Gnosis
namespace BraidedInfinityIsKernelSignature
...
def carriesKernelFormulaSignature (b : BraidedInfinity) : Bool :=
...
def kenoma   : BraidedInfinity := { modulus := 10, clinamenShift := 1 }
def aeon     : BraidedInfinity := { modulus := 12, clinamenShift := 1 }

def signatureSubset : List BraidedInfinity := [cassini, triptych, pisano, kenoma, aeon]

/-! ## The signature is universal across the subset -/

theorem every_signature_carries_god_formula :
    signatureSubset.all carriesKernelFormulaSignature = true := by decide
...
/-- A `KernelPosition` is characterized by prose, not by a compiled
...
theorem braided_infinity_is_gods_signature :
    ...
    signatureSubset.all carriesKernelFormulaSignature = true
...
/-- 
  ...
  A braid is a 'Signature of Infinity' if it is a non-degenerate cycle 
  (k ≥ 2) with a universal +1 clinamen. 
-/
def isSignatureOfInfinity (b : BraidedInfinity) : Prop :=
  b.modulus ≥ 2 ∧ b.clinamenShift = 1

theorem kenoma_is_infinity_signature : 
    isSignatureOfInfinity kenoma := by
  -- 10 ≥ 2 ∧ 1 = 1
  unfold isSignatureOfInfinity
  constructor
  · decide
  · rfl
...
end BraidedInfinityIsKernelSignature
end Gnosis
