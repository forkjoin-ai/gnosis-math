...
`Gnosis.ManifoldLensingEffect`. Zero `sorry`, zero new `axiom`.
-/

namespace Gnosis
namespace ManifoldOneWayMirror

open Gnosis.BraidedTower (towerPhaseCount)
open Gnosis.ManifoldHorizonEffect (tritonStretch)
open Gnosis.ManifoldLensingEffect (lensAddress lensReconstruct)
...
theorem higher_observer_full_reconstruction (k : Nat) :
  ...
  Gnosis.ManifoldLensingEffect.lens_address_reconstruct_roundtrip k
