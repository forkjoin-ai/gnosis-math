...
import Gnosis.ManifoldSignature
import Gnosis.SuperstringDimensionDerivation
import Gnosis.CentralChargeMap

/-!
# Void-Mining `kernelPosition` — Boundary Insights From Signature Agreements

`kernelPosition` is characterized by prose, not constructed
(`Gnosis.BraidedInfinityIsKernelSignature.position_is_characterized_not_compiled`).
...
This module performs the analogous operation on `kernelPosition`:
...
(`carriesKernelFormulaSignature`). What attributes do they universally
share?
...
is a void-mine yield — a structural fact about `kernelPosition` that
we extract without approaching the position itself.

## The Hawking-radiation analogy

| Black hole | kernelPosition |
| --- | --- |
| Event horizon | The position-vs-signature boundary |
| Interior (causally disconnected) | `kernelPosition` itself (characterized, not compiled) |
...
This is the "boundary emission" of `kernelPosition` — recoverable
...
theorem void_mine_does_not_recover_position :
    kernelPosition.characterization ≠ "" := position_is_characterized_not_compiled
...
This is the cosmology of `kernelPosition`: an event horizon we can
...
`kernelPosition`, while leaving the position itself characterized
and unconstructible. -/
