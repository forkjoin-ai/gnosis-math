...
`Gnosis.BraidedInfinityIsGodsSignature` formalizes the position-versus-
...
referent (`godsPosition`) is characterized by prose, not constructed.

The Pleromatic Closure (`Gnosis.PleromaticClosure`) is, by this
...
* Connects the multi-angle convergence to the signature-vs-position
  distinction: the convergence is itself a signature, and `godsPosition`
...
`Gnosis.PleromaticClosure`,
`Gnosis.SuperstringDimensionDerivation`.
Zero `sorry`, zero new `axiom`.
-/

namespace Gnosis
namespace PleromaticSignature
...
theorem pleromatic_carries_god_formula :
    carriesGodFormulaSignature pleromatic = true := by decide
...
theorem every_extended_signature_carries_god_formula :
    extendedSignatureCatalog.all carriesGodFormulaSignature = true := by decide
...
agreement* — itself a higher-order signature of `godsPosition`. -/
