...
Imports `Gnosis.BraidedTower`, `Gnosis.PleromaticClosure`,
`Gnosis.PleromaticHorizonEffect`. Zero `sorry`, zero new `axiom`.
-/

namespace Gnosis
namespace PleromaticHigherClosures
...
theorem closure_chain_zero_is_pleromatic :
  ...
  (Gnosis.PleromaticClosure.pleromatic_closure_point_is_ten).symm
