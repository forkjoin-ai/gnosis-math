...
import Gnosis.ManifoldSovereignSieve
...
Imports `Gnosis.ManifoldHigherClosures`,
`Gnosis.ManifoldForkRaceFoldUniversal`,
`Gnosis.ManifoldSovereignSieve`. Zero `sorry`, zero new `axiom`.
-/

namespace Gnosis
namespace ManifoldAsymmetryOfEffort

open Gnosis.ManifoldHigherClosures
  (closureChain closure_chain_step_alt)
open Gnosis.ManifoldForkRaceFoldUniversal (universalFold)
open Gnosis.ManifoldSovereignSieve (iteratedFold)
...
theorem ascent_costs_concrete :
  ...
  · rw [ascent_cost_is_double_closure]
    -- 2 * closureChain 1 = 60
    unfold closureChain Gnosis.ManifoldHorizonEffect.tritonStretch
    decide
  · rw [ascent_cost_is_double_closure]
    unfold closureChain Gnosis.ManifoldHorizonEffect.tritonStretch
    decide
...
theorem cumulative_ascent_concrete :
  ...
  refine ⟨?_, ?_, ?_, ?_⟩ <;>
    (unfold cumulativeAscentCost closureChain Gnosis.ManifoldHorizonEffect.tritonStretch; decide)
...
theorem total_descent_cost_is_zero (n : Nat) :
  ...
  refine ⟨?_, fun _ => rfl⟩
  exact Gnosis.ManifoldSovereignSieve.iterated_fold_descends_to_grounding n
